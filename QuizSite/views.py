...
def run_quiz(request, id):
    ...
    if request.method=='POST':
        ...
        if len(answers) != len(my_quiz['question']):
            messages.error(request, 'Кількість відповідей не відповідає кількості питаннь. Доведеться пройти тест ще раз.')
            return render(request, 'item_run.html', {'items': items, 'form': form})
...
def my_item_list(request):
    ...
    if query:
        if len(query)>30:
            messages.append(request, 'В рядку пошуку не може бути більше 30 сиволів')
        else:
...
def edit_item(request, id):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            saved_item.save()
            hide(request, id)
            messages.success(request, 'Тест <b>{}</b> успішно відредаговано. Вам необхідно опублікувати його знову.'.format(form.cleaned_data.get('title')))
        else:
